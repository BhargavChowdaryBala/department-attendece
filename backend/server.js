...
app.post('/api/spot-register', async (req, res) => {
  const { name, rollNo, branch, semester, password } = req.body;
  ...
  if (!name || !rollNo || !branch || !semester) {
    return res.status(400).json({ error: 'Name, Roll Number, Branch, and Semester are required' });
  }

  return withLock(async () => {
    console.log(`\n[SPOT] Processing Request for: "${name}" (${rollNo})`);
    try {
      const rows = await getCachedRows(true);
      ...
      if (studentRow) {
        studentRow.set('isPresent', 'TRUE');
        if (semester) studentRow.set('semester', semester);
        await studentRow.save();
        lastCacheUpdate = 0;
